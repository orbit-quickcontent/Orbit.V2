...
class _BookingScreenState extends ConsumerState<BookingScreen> {
  ...
  void initState() {
    ...
    if (p is Map) {
      package = Map<String, dynamic>.from(p);
    }
    _locate();
  }
  ...
  Future<void> _locate() async {
    try {
      if (!await Geolocator.isLocationServiceEnabled()) {
        setState(() => location = 'Location services are off');
        return;
      }
      ...
      if (permission == LocationPermission.denied || permission == LocationPermission.deniedForever) {
        setState(() => location = 'Location permission required');
        return;
      }
      ...
      setState(() {
        lat = pos.latitude;
        lng = pos.longitude;
        location = 'Current location ready';
      });
    } catch (_) {
  ...
  Future<void> _start() async {
    ...
    try {
      ...
      final order = await api.createPaymentOrder(
        amount: (package!['price'] as num?)?.toDouble() ?? 0,
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          if (package == null) ...[
            const Text(
              'Choose a package from Home first.',
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    package!['name']?.toString() ?? 'ORBIT Reel',
                    style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w800),
                  ),
                  const SizedBox(height: 8),
                  Text(
                    '₹${package!['price'] ?? '—'} • ${package!['deliveryTime'] ?? 'Fast delivery'}',
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      labelText: 'Shoot notes / creative brief',
                      hintText: 'Tell the partner what you want to capture…',
                    ),
                  ),
                ],
              ),
            ...
            FilledButton(
              ...
              child: Padding(
                ...
                child: Text(busy ? 'Preparing secure checkout…' : 'Pay securely with Razorpay'),
              ),
            ),
            const SizedBox(height: 10),
            const Text(
              'Your booking remains PENDING until Razorpay confirms payment. Partner dispatch begins only after verified payment.',
