...
enum class OrbitIconType {
    ...
    Palette, Tie, TheaterMasks, Compass, Person, Frame, Rocket
}
...
fun OrbitIcon(
    ...
    Canvas(modifier = modifier) {
        ...
        when (type) {
            ...
            OrbitIconType.Person -> {
                drawCircle(color = color, radius = w * 0.18f, center = Offset(w * 0.5f, h * 0.32f), style = stroke)
                val shoulders = Path().apply {
                    moveTo(w * 0.22f, h * 0.86f)
                    cubicTo(w * 0.22f, h * 0.60f, w * 0.78f, h * 0.60f, w * 0.78f, h * 0.86f)
                }
            ...
            OrbitIconType.Frame -> {
                ...
                drawCircle(color = color, radius = w * 0.09f, center = Offset(w * 0.36f, h * 0.38f), style = stroke)
            ...
            OrbitIconType.Rocket -> {
                val body = Path().apply {
                    moveTo(w * 0.5f, h * 0.08f)
                    cubicTo(w * 0.72f, h * 0.28f, w * 0.72f, h * 0.60f, w * 0.62f, h * 0.80f)
                    lineTo(w * 0.38f, h * 0.80f)
                    cubicTo(w * 0.28f, h * 0.60f, w * 0.28f, h * 0.28f, w * 0.5f, h * 0.08f)
                    close()
                }
                drawPath(body, color = color, style = stroke)
                drawCircle(color = color, radius = w * 0.08f, center = Offset(w * 0.5f, h * 0.36f), style = stroke)
                val finLeft = Path().apply {
                    moveTo(w * 0.38f, h * 0.68f)
                    lineTo(w * 0.22f, h * 0.86f)
                    lineTo(w * 0.38f, h * 0.80f)
                }
                drawPath(finLeft, color = color, style = stroke)
                val finRight = Path().apply {
                    moveTo(w * 0.62f, h * 0.68f)
                    lineTo(w * 0.78f, h * 0.86f)
                    lineTo(w * 0.62f, h * 0.80f)
                }
                drawPath(finRight, color = color, style = stroke)
            }
        }
    }
}
